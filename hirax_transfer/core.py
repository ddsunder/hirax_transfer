...
class HIRAXSurvey(HIRAXSinglePointing):
    ...
    def FoV(self):
        c = 299792458
        return (c/self.frequencies/1e6/self.dish_width)**2
    
    @property
    def survey_area(self):
        """
        Approximates survey area in square degrees
        """
        ddec = -1*np.deg2rad(self.pointings)+self.zenith[0]
        radius = np.sum(np.sin(ddec))
        return np.mean(2*np.pi*radius*np.sqrt(self.FoV))* (180/np.pi)**2;
